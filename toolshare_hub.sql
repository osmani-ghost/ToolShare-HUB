...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 27, 2026 at 10:33 AM
...
(8, 'Abul Kalam', 'abul@email.com', '01711000008', '$2y$10$evC22VCf0qf72ybXWXRhPOB7XJt223l4SwrPSpg7PaZ9KZQOlalbq', 2, 3.50, 3, 0, 'I am a mew', 'Member', '2024-03-20 14:00:00'),
...
CREATE TRIGGER `after_return_log` AFTER INSERT ON `return_log` FOR EACH ROW BEGIN
    -- Declare variables at the top (MySQL requirement)
    DECLARE v_tool_id    INT;
    DECLARE v_has_damage INT DEFAULT 0;

    -- Step 1: Get the tool_id from the loan record
    SELECT tool_id INTO v_tool_id
    FROM Loan_Record
    WHERE loan_id = NEW.loan_id;

    -- Step 2: Check if a Damage_Report already exists for this loan
    SELECT COUNT(*) INTO v_has_damage
    FROM Damage_Report
    WHERE loan_id = NEW.loan_id;

    -- Step 3: Set tool status based on return condition AND existing damage reports
    -- If condition is 'Damaged' OR a damage report was filed → Needs Inspection
    -- Otherwise → Available (safe to lend again)
...
--
-- Table structure for table `tool_condition_log`
--
...
--
-- Dumping data for table `tool_condition_log`
--
...
ALTER TABLE `borrow_request`
  ...
  ADD KEY `idx_borrow_tool` (`tool_id`),
  ADD KEY `idx_borrow_requester` (`requester_id`);
...
ALTER TABLE `damage_report`
  ...
  ADD KEY `idx_damage_loan` (`loan_id`);
...
ALTER TABLE `review`
  ...
  ADD UNIQUE KEY `uq_review_per_user_loan` (`loan_id`,`reviewer_id`),
  ADD KEY `reviewer_id` (`reviewer_id`),
  ADD KEY `idx_review_reviewee` (`reviewee_id`);
...
ALTER TABLE `tool`
  ...
  ADD KEY `idx_tool_owner` (`owner_id`),
  ADD KEY `idx_tool_status` (`status`);

--
-- Indexes for table `tool_condition_log`
--
ALTER TABLE `tool_condition_log`
  ADD PRIMARY KEY (`cond_id`),
  ADD KEY `idx_condlog_tool` (`tool_id`);
...
--
-- AUTO_INCREMENT for table `tool_condition_log`
--
ALTER TABLE `tool_condition_log`
  MODIFY `cond_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
...
--
ALTER TABLE `borrow_request`
  ADD CONSTRAINT `borrow_request_ibfk_1` FOREIGN KEY (`tool_id`) REFERENCES `tool` (`tool_id`),
  ADD CONSTRAINT `borrow_request_ibfk_2` FOREIGN KEY (`requester_id`) REFERENCES `member` (`member_id`);

--
-- Constraints for table `damage_report`
--
ALTER TABLE `damage_report`
  ADD CONSTRAINT `damage_report_ibfk_1` FOREIGN KEY (`loan_id`) REFERENCES `loan_record` (`loan_id`),
  ADD CONSTRAINT `damage_report_ibfk_2` FOREIGN KEY (`reported_by`) REFERENCES `member` (`member_id`);

--
-- Constraints for table `loan_record`
--
ALTER TABLE `loan_record`
  ADD CONSTRAINT `loan_record_ibfk_1` FOREIGN KEY (`tool_id`) REFERENCES `tool` (`tool_id`),
  ADD CONSTRAINT `loan_record_ibfk_2` FOREIGN KEY (`borrower_id`) REFERENCES `member` (`member_id`),
  ADD CONSTRAINT `loan_record_ibfk_3` FOREIGN KEY (`req_id`) REFERENCES `borrow_request` (`req_id`);

--
-- Constraints for table `member`
--
ALTER TABLE `member`
  ADD CONSTRAINT `member_ibfk_1` FOREIGN KEY (`area_id`) REFERENCES `area` (`area_id`);

--
-- Constraints for table `return_log`
--
ALTER TABLE `return_log`
  ADD CONSTRAINT `return_log_ibfk_1` FOREIGN KEY (`loan_id`) REFERENCES `loan_record` (`loan_id`);

--
-- Constraints for table `review`
--
ALTER TABLE `review`
  ADD CONSTRAINT `review_ibfk_1` FOREIGN KEY (`loan_id`) REFERENCES `loan_record` (`loan_id`),
  ADD CONSTRAINT `review_ibfk_2` FOREIGN KEY (`reviewer_id`) REFERENCES `member` (`member_id`),
  ADD CONSTRAINT `review_ibfk_3` FOREIGN KEY (`reviewee_id`) REFERENCES `member` (`member_id`);

--
-- Constraints for table `tool`
--
ALTER TABLE `tool`
  ADD CONSTRAINT `tool_ibfk_1` FOREIGN KEY (`owner_id`) REFERENCES `member` (`member_id`),
  ADD CONSTRAINT `tool_ibfk_2` FOREIGN KEY (`category_id`) REFERENCES `category` (`category_id`);

--
-- Constraints for table `tool_condition_log`
--
ALTER TABLE `tool_condition_log`
  ADD CONSTRAINT `tool_condition_log_ibfk_1` FOREIGN KEY (`tool_id`) REFERENCES `tool` (`tool_id`);
COMMIT;
